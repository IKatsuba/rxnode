...
export const access = observablify<[path: PathLike, mode?: number | undefined]>(originalAccess);

export const appendFile = observablify<[file: PathLike | number, data: any, options?: WriteFileOptions]>(originalAppendFile);

export const chmode = observablify<[path: PathLike, mode: string | number]>(originalChmode);

export const chown = observablify<[path: PathLike, uid: number, gid: number]>(originalChown);

export const close = observablify<[fd: number]>(originalClose);

export const copyFile = observablify<[src: PathLike, dest: PathLike, flags?: number]>(originalCopyFile);

export const fchmod = observablify<[fd: number, mode: string | number]>(originalFchmod);

export const fchown = observablify<[fd: number, uid: number, gid: number]>(originalFchown);
...
export const ftruncate = observablify<[fd: number, len?: number | undefined | null]>(originalFtruncate);

export const futimes = observablify<[fd: number, atime: string | number | Date, mtime: string | number | Date]>(originalFutimes);

export const lchmod = observablify<[path: PathLike, mode: string | number]>(originalLchmod);

export const lchown = observablify<[path: PathLike, uid: number, gid: number]>(originalLchown);

export const link = observablify<[existingPath: PathLike, newPath: PathLike]>(originalLink);

export const lstat = observablify<[path: PathLike], [Stats]>(originalLstat);

export const mkdir = observablify<[path: PathLike, options?: number | string | MakeDirectoryOptions | undefined | null]>(originalMkdir);

export const mkdtemp = observablify<[prefix: string, options?: 'buffer' | { encoding?: 'buffer' } | { encoding?: BufferEncoding | null } | BufferEncoding | undefined | null], [folder: string | Buffer]>(originalMkdtemp);

export const open = observablify<[path: PathLike, flags: string | number, mode?: string | number | undefined | null], [fd: number]>(originalOpen);
...
export const realpath = observablify<[path: PathLike, options?: { encoding?: BufferEncoding | null } | BufferEncoding | undefined | null | { encoding?: 'buffer' } | 'buffer' | string], [resolvedPath: Buffer | string]>(originalRealpath);

export const rename = observablify<[oldPath: PathLike, newPath: PathLike]>(originalRename);

export const rmdir = observablify<[path: PathLike]>(originalRmdir);

export const stat = observablify<[path: PathLike], [stats: Stats]>(originalStat);

//todo(fs): implemet symlink.Type
export const symlink = observablify<[target: PathLike, path: PathLike, type?: originalSymlink.Type | undefined | null]>(originalSymlink);

export const truncate = observablify<[path: PathLike, len?: number | undefined | null]>(originalTruncate);

export const unlink = observablify<[path: PathLike]>(originalUnlink);

export function unwatchFile(filename: PathLike, listener?: (curr: Stats, prev: Stats) => void): Observable<void> {
  return defer(() => originalUnwatchFile(filename, listener));
}

export const utimes = observablify<[path: PathLike, atime: string | number | Date, mtime: string | number | Date]>(originalUtimes);
